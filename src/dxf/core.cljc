(ns dxf.core
  "DXF (AutoCAD Drawing Exchange Format) as data — 'hiccup for drawings'. DXF on disk is a flat stream
   ...
   the kotoba.* family (complements kotoba.scad on the CAD axis). `.cljc`.

   DXF is pair-oriented (not infix/tree), so it does not use kotoba.expr. An entity is `[:kind {attrs}]`;
